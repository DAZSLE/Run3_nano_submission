"""
Customizations for NanoAOD v14
Based on https://github.com/cms-tau-pog/NanoProd/blob/main/NanoProd/python/customize.py, but with no cuts applied.
"""

import FWCore.ParameterSet.Config as cms
from PhysicsTools.NanoAOD.common_cff import Var, CandVars
from RecoTauTag.RecoTau.tauIdWPsDefs import WORKING_POINTS_v2p5
from PhysicsTools.NanoAOD.nano_eras_cff import run3_common
from Configuration.DataProcessing.Utils import addMonitoring

# settings which can depend on (nanoAOD) era
# center of mass energy in GeV (Run3 default)
...
def customizeTaus(process):
    ...
    process.tauTable.variables.ip3dErr = Var(
        "ip3d_error", float, doc="3D impact parameter error", precision=10
    )
    process.tauTable.variables.hasSV = Var("hasSecondaryVertex", bool, doc="has secondary vertex")
    process.tauTable.variables.flightLengthX = Var(
        "flightLength().x()", float, doc="flight length x", precision=10
    )
    process.tauTable.variables.flightLengthY = Var(
        "flightLength().y()", float, doc="flight length y", precision=10
    )
    process.tauTable.variables.flightLengthZ = Var(
        "flightLength().z()", float, doc="flight length z", precision=10
    )
    process.tauTable.variables.flightLengthSig = Var(
        "flightLengthSig()", float, doc="flight length significance", precision=10
    )

    process.tauTable.variables.dzErr = Var(
        "?leadChargedHadrCand.isNonnull() && leadChargedHadrCand.hasTrackDetails()?leadChargedHadrCand.dzError(): 0/0.",
        float,
        doc="dz error",
        precision=10,
        lazyEval=True,
    )
    process.tauTable.variables.leadTkNormChi2 = Var(
        "leadingTrackNormChi2()", float, doc="normalized chi2 of the leading track", precision=10
    )
    process.tauTable.variables.leadChCandEtaAtEcalEntrance = Var(
        "etaAtEcalEntranceLeadChargedCand",
        float,
        doc="eta of the leading charged candidate at the entrance of the ECAL",
        precision=10,
    )
...
def customizePV(process):
    ...
    process.pvbsTable.variables.ndof = Var(
        "ndof()", float, doc="number of degrees of freedom", precision=8
    )
...
def addIPCovToLeptons(process, lepton="all"):
    ...
    for input_obj, input_col in inputs:
        if (lepton == input_obj or lepton == "all") and hasattr(
            process, f"{input_col}TimeLifeInfoTable"
        ):
...
def customize(process):
    ...
    # default customization recommended here https://gitlab.cern.ch/cms-nanoAOD/nanoaod-doc/-/wikis/Instructions/Private-production
    ...
    # process = addSpinnerWeights(process)  # Leads to errors, and probably not needed
