...
def get_crab_info(directory):
    try:
        ...
        result = subprocess.run(
            ["crab", "status", "-d", directory], capture_output=True, text=True, check=True
        )
...
def main(base_path, output_path, yaml_path, csv_ext):
    ...
    yaml_list = (
        [item.strip() for item in yaml_list_str.split("\n")]
        if isinstance(yaml_list_str, str)
        else []
    )
    ...
    directory_list = [
        ...
        if os.path.isdir(os.path.join(base_path, d)) and "crab" in d
    ]
    data_list = []
    job_status = []
    sample_name = []
    ...
    print(
        f"csv file created and saved in the www eos area. Every execution will overwrite the previous csv.: {csv_file}"
    )
    # print(data_list)
